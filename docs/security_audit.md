...
- Google tag нельзя загружать до согласия.
- Page location должен передаваться без query и fragment.
- Event API должен принимать только фиксированные типы и параметры.
...
- CSP allowlist ограничивается фактически используемыми analytics domains.
